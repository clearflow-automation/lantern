...
test('Retired consulting URL redirects permanently to the consolidated homepage',async()=>{const r=await worker.fetch(new Request('https://airlantern.com/business-process-consulting/?utm_source=legacy'),env);assert.equal(r.status,301);assert.equal(r.headers.get('Location'),'https://airlantern.com/?utm_source=legacy')});
