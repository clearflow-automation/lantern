...
// The approved full film replaces the earlier timed SVG walkthrough.
const lanternFilm = document.getElementById('lantern-film');
const lanternFilmPlay = document.querySelector('.film-play-overlay');
const lanternFilmError = document.getElementById('film-error');
if (lanternFilm && lanternFilmPlay) {
  const syncFilmButton = () => {
    lanternFilmPlay.hidden = !lanternFilm.paused && !lanternFilm.ended;
    lanternFilmPlay.setAttribute('aria-label', lanternFilm.ended ? 'Replay the Lantern video, 2 minutes 39 seconds' : 'Play the Lantern video, 2 minutes 39 seconds');
  };
  const startLanternFilm = () => {
    lanternFilmError.hidden = true;
    if (lanternFilm.ended) lanternFilm.currentTime = 0;
    lanternFilm.play().then(() => lanternFilm.focus({preventScroll:true})).catch(() => {
      syncFilmButton();
      lanternFilmError.hidden = false;
    });
  };
  lanternFilmPlay.addEventListener('click', startLanternFilm);
  document.querySelectorAll('[data-play-lantern-film]').forEach(link => link.addEventListener('click', event => {
    event.preventDefault();
    lanternFilm.scrollIntoView({behavior:'auto',block:'center'});
    startLanternFilm();
  }));
  ['play','pause','ended'].forEach(event => lanternFilm.addEventListener(event, syncFilmButton));
  lanternFilm.addEventListener('error', () => { lanternFilmError.hidden = false; syncFilmButton(); });
  syncFilmButton();
}
